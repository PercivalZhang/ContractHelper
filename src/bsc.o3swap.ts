...
const getLPVaultDetail = async (vaultAddress: string, lptAddress: string, userAddress: string) => {
    ...
    const myLPTBalance = new BigNumber(
        await vault.callReadMethodWithFrom(
            'getStaked',
            userAddress,
            lptAddress, // LP O3-BUSD(BSC)
        ),
    );
    logger.info(
        `staked balance: ${myLPTBalance.dividedBy(Math.pow(10, 18)).toNumber().toFixed(6)} ${lptDetail.token1.symbol}-${
            lptDetail.token0.symbol
        } LP`,
    );
